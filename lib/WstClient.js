  ...
  start(localHost, localPort, wsHostUrl, remoteAddr, optionalHeaders, cb) {
    ...
    this.udpServer.on('listening', () => {
      ...
      this._connect(
        ...
        (err, stream) => {
          if (err) this.emit('connectFailed', err);
          else bind(this.udpServer, stream);
        }
      );
    });

    this.udpServer.bind(localPort, localHost, () => {
      console.log("Server bound to port " + this.udpServer.address().port);
    });
