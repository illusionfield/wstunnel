      ...
      const handleReq = (request, connWrapperCb) => {
        ...
        return this.authenticate(
          ...
          (rejectReason, target, monitor) => {
            ...
            const bindPort = parseInt(((request.resourceURL || {}).query || {}).port || "") || undefined;

            var udpConn = udp.createSocket('udp4');

            udpConn.on('error', (err) =>{
            //   request.reject(
            //     500,
            //     JSON.stringify(
            ...
            udpConn.on('connect', () => {
              const ip = require('./httpReqRemoteIp')(httpRequest);
                ...
                bindStream(wsConn, udpConn);
                if (monitor) {
                  return monitor.bind(wsConn, udpConn);
                }
            });

            console.log({bindPort, port, host});
            
            udpConn.bind(bindPort);
            udpConn.connect(port, host);
