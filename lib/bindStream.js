...
module.exports = function (s1, s2) {
  ...
  const stop = function () {
    if (!this._stop) {
      dlog(this, 'stop');
      this._stop = true;
      return this.end ? this.end() this.close();
    }
  };
  s1.stop = stop;
  s2.stop = stop;
  ...
  const manualPipe = function () {
    s1.on('data', function (data) {
      if(!s2._stop) {
        return s2.write ? s2.write(data) : s2.send(data, s2.address().port);
      }
    });
    s2.on('data', function (data) {
      if(!s1._stop) {
        return s1.write ? s1.write(data) : s1.send(data, s1.address().port);
      }
    });
